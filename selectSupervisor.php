	...
	<main>
		<div class="estilo">
			<div class="tit">
				...
				<br>
				<br>
				<h4><strng>SELECIONANDO SUPERVISOR(A)</strng></h4>
		</div>
		
		<br><br><br>
		<div class="texto1">
			<form action="upsupervisor.php" method="POST">
				<label>SELECIONE O(A) SUPERVISOR(A):</label> <br>
				<select name="supervisor" required>
					<option value="" disabled selected>Escolha o(a) supervisor(a) do estágio!</option>

					<?php
						$supervisor_sql = "SELECT * FROM supervisor WHERE status = '1' and codEmpresa = '$codigoempresa'";
						$supervisor_query = $mysqli->query($supervisor_sql);
						

						while ($supervisor = mysqli_fetch_assoc($supervisor_query)) {
							?><option value=<?php echo $supervisor['codSupervisor']; ?>><?php echo $supervisor['nome']; ?></option><?php
						}
					?>
					
				</select>
				<br><br>
				<p> Caso o(a) supervisor(a) não esteja na lista,<br><a href="cadSupervisor.php"> clique aqui para Cadastrar!</a></p>
				<br><br>
