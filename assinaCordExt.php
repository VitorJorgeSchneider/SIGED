...
$sql_code = "SELECT * FROM assinantes WHERE cargo = 'Coordenador de extensão'";
$sql_query = $mysqli->query($sql_code) or die("Falha na execução do código SQL: " . $mysqli->error);
$cordext = $sql_query->fetch_assoc();
...
try {
    ...
    $mail->addAddress($cordext['email'], $cordext['nome']);     
    
    

    $mail->isHTML(true);                                  
    $mail->Subject = 'Solicitação de Assinatura do Termo de Compromisso!';
    $mail->Body = "Olá! ". $cordext['nome'] ." <br> Existe uma solicitação de assinatura para você!<a href = "."http://168.228.253.7/~vitor/assCordExt.php?aluno=$hash1".">Clique aqui!</a>
     Para ser redirecionado(a) ao portal! Não compartilhe este link com ninguém!";
    $mail->send();
	header("Location: assinaDiretor.php");
} catch (Exception $e) {
