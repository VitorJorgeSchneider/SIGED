...
<body id="body">
	<main id="conteudo">
		<div class="estilo">
			...
			<div class="texto2">	
				...
				<div class="boxassin">
					...
					<div class="boxass1">
						<?php if ($estagio['assinaturaDiretor'] == NULL) {
							?><hr>
							<p>Entidade Educacional</p><?php
						}else{
							$data_ass_dir = strtotime($estagio['assinaturaDiretor']);
							$data_assinada_dir = date("d/m/Y h:i:s", $data_ass_dir);
							echo 'Assinado digitalmente por ' .$iff['nome'];?><br>
							<?php echo ' Data: ' .$data_assinada_dir; ?>
							<hr>
							<p>Entidade Educacional</p><?php
						}  ?>
					</div>
		...
		<?php if ($_SESSION['idset'] == true && $estagio['status'] == 'analise') {
			?><div class="assin_box">
			
				<form action ="submeterAssinaturas.php" method="POST">
					<button type="submit" name= "sub" value= <?php echo $matricula; ?>>Ok, Submeter Assinaturas!</button>
				</form>
			</div>
		<?php }
		if($_SESSION['idset'] == true && $estagio['status'] != 'completo'){
			?><br><br>
				<form action ="deletarDoc.php" method="POST">
					<button type="submit" name= "del" value= <?php echo $matricula; ?>>Está incorreto, Refazer!</button>
				</form><?php
		} ?>
